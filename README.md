...
- **一键铺点**：技能栏是等距的 —— 对准第一格按一次、最后一格按一次，中间的点自动铺开。
  8 个格子从点 8 次变成点 2 次。
- **一键学色**：把游戏摆成你想让它触发的样子，一个按钮把当前画面的颜色写进全部条件，
  不用手敲 `#RRGGBB`。
- **技能页**：规则的简化视图 —— `[✓] 追风剑 [2] 尾随0ms 冷却0ms [颜色] 学色`，
  勾选 + 调延迟就能用，不用进规则编辑器。
- **取色方案 1/2/3**：同一套取色点和规则，存三套颜色 —— 全屏 / 窗口 / 换了装备各一套。
- **左右键停宏**：按住鼠标左/右键时挂起，松开继续，方便转视角和走位。
...
| 技能 | 取色 | 规则 |
|---|---|---|
| ![](docs/img/skills.png) | ![](docs/img/probes.png) | ![](docs/img/rules.png) |
...
4. 把游戏摆成要触发的状态 →「技能」页点「一次学全部技能的色」→ 勾上要用的技能。
5. 回「运行」，按 **Ctrl+Q** 开始。

首次启动加载 `profiles/hotbar.template.json`，之后每次启动加载你上次用的那份。
...
`color_plan` / `color_sets` 是取色方案 1/2/3：同一套规则存三份颜色，切换即生效。
...
原版宏内置了**职业技能表**——技能名、默认键位、耗时和延迟都列好，用户勾选就行。
内置不了的是**颜色**：它 `data/ma_feng.json` 里的 `colors` 字段就是空的 `[{},{},{}]`，
说明书那句"对着怪按 F1 一下"，就是让用户自己取。坐标同理，取决于你的分辨率。

所以这里照同一个思路来，`profiles/` 下两份模板：

| 文件 | 内容 |
|---|---|
| `hotbar.template.json` | 通用骨架：8 个技能格 → 按键 1-8 |
| `lingjian-flash.template.json` | 灵剑-闪光：技能名/键位/耗时照抄自原版 v27.1 界面 |

两份都是**规则齐全、颜色待取**：容差 0（永不命中）、规则默认全关，
所以加载后直接按 Ctrl+Q 不会乱按键。三步变成你自己的宏：

1. 「取色点」页 **一键铺点** —— 重铺成你自己分辨率下的坐标
2. 把游戏摆成要触发的样子，「技能」页 **一次学全部技能的色**
3. 勾上要用的技能

其它职业没做，因为我手上只有灵剑-闪光这一份真实数据（你给的截图）。
把你那个职业的技能表截图发我，或者直接把技能名和键位列出来，照着加一份就行。
...
- 自动背包分解、自动换装、切频道、点龙脉
- 技能计时器（语音提示 / 打字提示）
- **BUFF 时效百分比**（原版的「闪电BUFF时效 70%」）：需要记录 buff 什么时候开始，
  本引擎没有这个状态，硬用冷却去凑是假的，所以干脆不做
